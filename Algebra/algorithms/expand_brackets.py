...
def get_answer():
    ...
    answer = str(num1*num1)+"x^2" + str(op) + \
        str(2*num1*num2)+"xy+" + str(num2*num2)+"y^2"
    return(answer)
...
def get_help(effort):
    ...
    if len(effort) < 6:
        # if they click on check without entering anything sufficient
        return("error, Your response is too short")
    ...
    if (effort.find(term3)) >= 0:
        ...
        # there must be a y^2 and the character before is a +
        if ((effort.find("y^2")) >= 0 and (effort[effort_findnum] == "+")):
            t3ok = 1
    ...
    if (t1ok == 1 and t2ok == 1 and t3ok == 1):
        ...
        if t2ok == 0:
            helptext = helptext+"\n\n You got the 2nd term wrong... The coefficient should be "+str(2*num1*num2)+" \n" \
                "which you get by multiplying 2 * :" + \
                str(num1) + " * "+str(num2) + " and then just type in xy"

        if t3ok == 0:
            helptext
            helptext = helptext+"\n\n You got the 3nd term wrong... It should be " + term3 + " the coefficient should be "+str(num2*num2)+" \n" \
                "which you get by multiplying :" + \
                str(num2) + " * "+str(num2) + " and then the y^2"
